...
- `src/core/secrets.ts` — Zod-validated `.env` (GEMINI_API_KEY, OPENWA_API_KEY). Both are required (no defaults). `OPENWA_API_KEY` is generated by the OpenWA server and auto-captured by `waify setup` from the container's `/app/data/.api-key` — there is no shared default, and it lives only in `.env` (not `config.json`).
